...
$urcheck = mysql_num_rows(mysql_query("SELECT `url` FROM `ads` WHERE `url`='$url'"));

$tcheck = mysql_num_rows(mysql_query("SELECT `title` FROM `ads` WHERE `title`='$title'"));
...
$add = mysql_query("INSERT INTO `ads` SET `img`='$img', `description`='$description', `boardid`='$on', `expiredate`='$expiredate', `title`='$title', `url`='$url', `date`='$date'");
if($add)
{
$info = mysql_fetch_array(mysql_query("SELECT `id` FROM `ads` WHERE `title`='$title'"));
$id = $info["id"];
functions::go("?action=view&id=$id");
} else {
$pagetitle = "Error Adding Ads";
require('../incfiles/head.php');
functions::display_error('Error Adding Ads');
require('../incfiles/end.php');
die();
}
} else {
$pagetitle = "Add Ads";
require('../incfiles/head.php');
echo "<p><h2>Add Ads</h2><p><div class='display'><form action='#' method='POST'>
...
$urcheck = mysql_num_rows(mysql_query("SELECT `url` FROM `ads` WHERE `url`='$url' AND `id`!='$id'"));

$tcheck = mysql_num_rows(mysql_query("SELECT `title` FROM `ads` WHERE `title`='$title' AND `id`!='$id'"));
...
$edit = mysql_query("UPDATE `ads` SET `img`='$img', `description`='$description', `boardid`='$on', `title`='$title', `url`='$url'  WHERE `id`=$id");

if($edit)
{
functions::go("?action=view&id=$id");
} else {
$pagetitle = "Error Editing Ads";
require('../incfiles/head.php');
functions::display_error('Error Editing Ads');
require('../incfiles/end.php');
die();
}
} else {
$pagetitle = "Edit Ads";
require('../incfiles/head.php');
echo "<p><h2>Edit Ads</h2><p><div class='display'><form action='#' method='POST'>
...
$renew = mysql_query("UPDATE `ads` SET `expiredate`='$expiredate' WHERE `id`=$id");

if($renew)
{
functions::go("?action=view&id=$id");
} else {
$pagetitle = "Error Renewing Ads";
require('../incfiles/head.php');
functions::display_error('Error Editing Ads');
require('../incfiles/end.php');
die();
...
$pagetitle = "Renew Ads $atitle";
require('../incfiles/head.php');
echo "<p><h2>Renew Ads $atitle</h2><p><div class='display'><form action='#' method='POST'>
<b>Expiring Time: (in days)</b><p><input type='text' name='time' value='1' size='4'><br>
...
if($verify == yes)
{
$del = mysql_query("DELETE FROM `ads` WHERE `id`='$id'");
if($del)
{
$pagetitle = "Ads Successfully Deleted";
require('../incfiles/head.php');
functions::display_error('Ads Successfully Deleted<p><a href="/panel/ads">GO BACK</a>');
require('../incfiles/end.php');
die();
} else {
$pagetitle = "Error Deleting Ads";
require('../incfiles/head.php');
functions::display_error('Error Deleting Ads');
require('../incfiles/end.php');
die();
}

} else {
functions::go('/panel/ads');
}
} else {
...
$pagetitle = "View Ads: $title";
require('../incfiles/head.php');
echo '<p><h2>View Ads: '.$title.'</h2><p><div class="display">
<div class="ratatamp"><a href="/click?id='.$id.'" rel="nofollow"><img src="'.$img.'"></a></div>
...
require('../incfiles/head.php');
echo '<p><h2>Manage Ads</h2><p><a href="/">'.$config->title.' Forum</a> / <a href="/panel/">Panel</a> / <a href="#">Manage Ads</a><p>';
...
$numrows = mysql_num_rows(mysql_query("SELECT `img`,`id`,`expiredate` FROM `ads`"));
...
$query = mysql_query("SELECT `img`,`id`,`expiredate` FROM `ads` LIMIT $offset, $rowsperpage"); 

$pagination= new pagination($config->postsperpage, $page, '/recent/(page)', $numrows);

if($numrows < 1)
{
echo '<div class="display">No Ads Yet<p><a href="?action=add">Creat New Ads</a></div>';
...
<div class="ratatamp"><a href="/click?id='.$id.'" rel="nofollow"><img src="'.$img.'"></a></div><p>Status: '.$status.'<p><a href="?action=edit&id='.$id.'">edit</a> - <a href="?action=delete&id='.$id.'">delete</a> - <a href="?action=view&id='.$id.'">view</a>';
$i++;
}
echo '</table>';
echo ''.$pagination->display().'';
