...
$limit = 15;
$urows = mysql_num_rows( mysql_query("SELECT * FROM `down` WHERE type='6'"));
$pagination2 = new pagination($limit, 1, ''.$config->url.'clickdown/(page)', $urows);

$adsquery = mysql_query("SELECT `id`,`img`,`title`, `url` FROM down WHERE `type`='6' ORDER BY `id` DESC LIMIT 15");
