...
$viewingtopic = functions::cleanlast($viewingtopic, 2);
}
...
echo '<h2>'.$title.' - '.$fname.' - '.$config->title.'</h2><p class="bold"><a href="/">'.$config->title.' Forum</a>'.$typename.' / <a href="'.urls::board($fname, $fid).'">'.$fname.'</a> / <a href="'.urls::topic($title, $id).'">'.$title.'</a> ('.$views.' Views)<p> '.functions::cleanlast($relout, 2).' '.$pagination2->display2().'<p>';
...
$lnk = "";
$sh = '<div class="a2a_kit a2a_kit_size_32 a2a_default_style">
...
if($admincheck > 0 || $modcheck > 0 || $modcheck2 > 0)
{
